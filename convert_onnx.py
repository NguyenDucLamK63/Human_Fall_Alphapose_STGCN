...
def main():
    ...
    inp_dets = 384
    device= 'cuda'
    weight_file = '/home/duclam/Lam/fall_detection/Human-Falling-Detect-Tracks_2/Models/TSSTG/model_raw/tsstg-model.pth'
    ...
    action_model.load_state_dict(torch.load(weight_file))
    # example_output = action_model(dummy_input)
    # print(example_output)
    action_model.eval()
    print("doneeeee")
    torch.onnx.export(action_model,         # model being run 
        args= (dummy_input_1,dummy_input_2),       # model input (or a tuple for multiple inputs) 
