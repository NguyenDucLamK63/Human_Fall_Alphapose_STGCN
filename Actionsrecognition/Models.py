...
class TwoStreamSpatialTemporalGraph(nn.Module):
    ...
    """
    """Hai đầu vào mạng tích chập đồ thị không gian thời gian.
    lập luận:
        - graph_args: (dict) Bản đồ Args của Lớp `Actionsrecognition.Utils.Graph`.
        - num_class: (int) Số đầu ra của lớp.
        - edge_importance_weighting: (bool) Nếu `True`, thêm tầm quan trọng có thể học được
            trọng số cho các cạnh của đồ thị.
        - **kwargs: (tùy chọn) Các tham số khác cho đơn vị tích chập đồ thị.
    Hình dạng:
        - Đầu vào: :bộ toán học:`((N, 3, T, V), (N, 2, T, V))`
        cho các điểm và dòng chuyển động ở đâu.
            :math:`N` là kích thước lô,
            :math:`in_channels` là các kênh dữ liệu (3 là (x, y, điểm)), (2 là (mot_x, mot_y))
            :math:`T` là độ dài của chuỗi đầu vào,
            :math:`V` là số nút biểu đồ,
        - Kết quả: :math:`(N, num_class)`"""
