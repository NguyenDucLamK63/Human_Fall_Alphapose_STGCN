    ...
    for human in im_res:  # ['result']:
        ...
        for i, (start_p, end_p) in enumerate(l_pair):
            if start_p in part_line and end_p in part_line:
                start_xy = part_line[start_p]
                end_xy = part_line[end_p]
                # print("2*(kp_scores[start_p] + kp_scores[end_p]) + 1 = ", 2*(kp_scores[start_p] + kp_scores[end_p]) + 1 , '\n')
                # print("2*(kp_scores[start_p] + kp_scores[end_p]) + 1 = ", int(2*(kp_scores[start_p] + kp_scores[end_p]) + 1))
