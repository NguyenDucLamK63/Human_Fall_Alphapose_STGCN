...
class TinyYOLOv3_onecls(object):
    ...
    def detect(self, image, need_resize=True, expand_bb=5):
        ...
        lập luận:
            hình ảnh: (mảng numpy) Hình ảnh RGB đơn để phát hiện.,
            Need_resize: (bool) Thay đổi kích thước thành input_size trước nguồn cấp dữ liệu và sẽ trả lại bbox
                với tỷ lệ thành kích thước ban đầu của hình ảnh.,
            expand_bb: (int) Mở rộng ranh giới của các hộp.
        Trả lại:
            (torch.float32) Trong mỗi đối tượng được phát hiện có chứa một
                [trên, trái, dưới, phải, bbox_score, class_score, class]
            trả về `None` nếu không phát hiện.'''
