...
class CamLoader:
    ...
    """
    """Sử dụng luồng để chụp khung hình từ máy ảnh để tải khung hình nhanh hơn.
    Đề xuất cho máy ảnh hoặc webcam.

    arg:
        máy ảnh: (int, str) Nguồn máy ảnh hoặc video.,
        tiền xử lý: (Hàm có thể gọi) để xử lý khung trước khi trả về."""
...
class CamLoader_Q:
    ...
    Args:
        ...
        preprocess: (Callable function) to process the frame before return.
    """
    '''Sử dụng threading và queue để chụp khung và lưu trữ để xếp hàng chờ nhận theo trình tự.
    Đề xuất cho tệp video.

    Args:
        máy ảnh: (int, str) Nguồn máy ảnh hoặc video.,
        batch_size: (int) Số lượng khung hàng loạt để lưu trữ trong hàng đợi. Mặc định: 1,
        queue_size: (int) Kích thước hàng đợi tối đa. Mặc định: 256,
        tiền xử lý: (Hàm có thể gọi) để xử lý khung trước khi trả về.'''
    ...
    def getitem(self):
        return self.Q.get().squeeze() # tach bo 1 chieu
